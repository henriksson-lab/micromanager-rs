...
impl PeconTempControl {
    pub fn new() -> Self {
        ...
        props
            ...
            .unwrap();
        props
            .define_property("Channel1_RealTemperature", PropertyValue::Float(0.0), true)
            .unwrap();
        props
    ...
    pub fn decode_temp(bytes: &[u8]) -> f64 {
        ...
        if bytes[0] == b'-' {
            -(((bytes[1].wrapping_sub(b'0')) as f64) + 0.1 * (bytes[2].wrapping_sub(b'0')) as f64)
        } else {
...
impl Default for PeconTempControl {
    fn default() -> Self {
        Self::new()
    }
}
...
impl Device for PeconTempControl {
    ...
    fn initialize(&mut self) -> MmResult<()> {
        ...
        self.real_temp[1] = t2;
        self.props
            .entry_mut("Channel1_RealTemperature")
            .map(|e| e.value = PropertyValue::Float(t1));
        self.props
            .entry_mut("Channel2_RealTemperature")
            .map(|e| e.value = PropertyValue::Float(t2));
...
mod tests {
    ...
    fn make_transport() -> MockTransport {
        // A000 → 3 bytes "000"; S000 → "010" means TempControl device code 1.
