...
impl YodnE600 {
    ...
    fn send_recv_fixed_bytes(&self, cmd: &[u8], resp_len: usize) -> MmResult<Vec<u8>> {
        let resp = self.send_recv_bytes(cmd, resp_len)?;
    ...
    fn get_lamp_state(&self) -> MmResult<u8> {
        let resp = self.send_recv_fixed_bytes(&[0x57, 0x00], 3)?;
        Ok(resp[2])
    }
    ...
    fn get_channel_intensity(&self, ch_idx: usize) -> MmResult<u8> {
        ...
        let resp = self.send_recv_fixed_bytes(&[0x56, ch_id], 3)?;
        Ok(resp[2])
    }
    ...
    fn get_channel_temperature(&self, ch_idx: usize) -> MmResult<u8> {
        let ch_id = CHANNEL_IDS[ch_idx];
        let resp = self.send_recv_fixed_bytes(&[0x55, ch_id], 3)?;
        Ok(resp[2])
    }
    ...
    fn get_channel_use_state(&self, ch_idx: usize) -> MmResult<u8> {
        let ch_id = CHANNEL_IDS[ch_idx];
        let resp = self.send_recv_fixed_bytes(&[0x57, ch_id], 3)?;
        Ok(resp[2])
    }
    ...
    fn get_channel_use_time(&self, ch_idx: usize) -> MmResult<u16> {
        let ch_id = CHANNEL_IDS[ch_idx];
        let resp = self.send_recv_fixed_bytes(&[0x53, ch_id], 4)?;
        let high = resp[2] as u16;
        let low = resp[3] as u16;
        Ok(high * 256 + low)
    }

    /// Get error code.
    fn get_error_code(&self) -> MmResult<u8> {
        let resp = self.send_recv_fixed_bytes(&[0x52], 2)?;
        Ok(resp[1])
    }
    ...
    fn define_runtime_property(
        ...
        if !self.props.has_property(name) {
            ...
            self.props.entry_mut(name).map(|entry| entry.value = value);
        }
        Ok(())
    }
...
impl Device for YodnE600 {
    ...
    fn initialize(&mut self) -> MmResult<()> {
        ...
        self.define_runtime_property("Main Version", PropertyValue::String(main_version), true)?;

        let mut panel_version = self.get_version(0x51)?;
        if !panel_version.is_empty() {
            panel_version.remove(0);
        }
        self.define_runtime_property("Panel Version", PropertyValue::String(panel_version), true)?;
        ...
        for i in 0..NUM_CHANNELS {
            ...
            self.define_runtime_property(
                &intensity_name,
                PropertyValue::Integer(intensity as i64),
                false,
            )?;
            self.props
        ...
        self.define_runtime_property("Lamp", PropertyValue::Integer(lamp as i64), false)?;
        self.props.set_allowed_values("Lamp", &["0", "1"])?;
        ...
        for i in 0..NUM_CHANNELS {
            ...
            self.define_runtime_property(
                &use_name,
                PropertyValue::Integer(use_state as i64),
                false,
            )?;
            self.props.set_allowed_values(&use_name, &["0", "1"])?;
        }
        ...
        for i in 0..NUM_CHANNELS {
            ...
            self.define_runtime_property(
                &temp_name,
                PropertyValue::Integer(Self::temperature_transform(temp)),
                true,
            )?;
        }
        ...
        for i in 0..NUM_CHANNELS {
            ...
            self.define_runtime_property(
                &time_name,
                PropertyValue::Integer(use_time as i64),
                true,
            )?;
        }
        ...
        self.define_runtime_property(
            "Error",
            PropertyValue::String(Self::error_code_str(err)),
            true,
        )?;
...
impl Shutter for YodnE600 {
    ...
    fn get_open(&self) -> MmResult<bool> {
        if self.initialized {
            return match self.get_lamp_state()? {
                0 => Ok(false),
                1 => Ok(true),
...
mod tests {
    ...
    fn initialize_no_error() {
        ...
        assert!(!dev.has_property("Main Version"));
        assert!(!dev.has_property("Lamp"));
        assert!(!dev.has_property("Intensity CH1"));
    ...
    #[test]
    fn busy_remains_false_like_upstream() {
        let t = make_transport();
        let mut dev = YodnE600::new().with_transport(Box::new(t));
        dev.initialize().unwrap();
    ...
    #[test]
    fn shutdown_does_not_rewrite_cached_open_state() {
        let t = make_transport();
        let mut dev = YodnE600::new().with_transport(Box::new(t));
        dev.initialize().unwrap();
    ...
    #[test]
    fn set_open_before_initialize_does_not_send_lamp_command() {
        let mut dev = YodnE600::new().with_transport(Box::new(MockTransport::new()));

        assert_eq!(dev.set_open(true), Err(MmError::CanNotSetProperty));
        assert!(!dev.get_open().unwrap());
    }

    #[test]
    fn get_open_rejects_invalid_lamp_state_like_upstream() {
        let t = make_transport().expect_binary(b"\x57\x00\x02");
        let mut dev = YodnE600::new().with_transport(Box::new(t));
        dev.initialize().unwrap();
    ...
    #[test]
    fn fixed_length_binary_reads_reject_short_responses() {
        let t = make_transport().expect_binary(b"\x57\x00");
        let mut dev = YodnE600::new().with_transport(Box::new(t));
        dev.initialize().unwrap();
