...
///
/// Protocol:
///   Set position:  device=rLFA(67/12), command=2, data=pos  -> `"<DD>002<pos>\r"`
...
impl LeicaDMRTurret {
    /// `device_id`: rLFA4=67, rLFA8=12, ObjNosepiece=20
...
mod tests {
    ...
    fn rl_turret_initialize_and_move() {
        // device=67 (rLFA4), init: get pos -> 1 (0-based 0), move to pos 2 -> send 3
        let t = MockTransport::new()
            .expect("67010\r", "670101") // get pos: Leica 1-based 1 -> 0-based 0
            .expect("670023\r", "670023"); // set to pos 2 -> send Leica 3
    ...
    fn label_navigation() {
        let t = MockTransport::new()
            .expect("67010\r", "670101")
            .expect("670023\r", "670023");
        let mut turret = LeicaDMRTurret::new(67, 4).with_transport(Box::new(t));
        turret.initialize().unwrap();
